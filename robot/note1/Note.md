...
1. 动作执行
   2. Generative Imitation Learning：直接去学习每个joint的轨迹
...
1. 控制方法：前两者仅适用于步态很慢的工况
   2. ZMP：零力矩点控制![alt text](image-8.png)
   3. SLIP：把整个机器人抽象成一个弹簧倒立摆，通过控制弹簧的伸缩和速度来控制倒立摆![alt text](image-9.png)
   4. MPC：![alt text](image-10.png) 
5. 模型分类：![alt text](model_classification.jpg)
   6. 全身动力学（WBD）：
...
1. MPC建模（面足式单刚体动力学模型）：
   ...
   3. 系统动力学方程![alt text](image-25.png)四个状态量分别是欧拉角，质心位置，角速度和质心速度![alt text](image-26.png)前向欧拉离散化![alt text](image-27.png)
   4. 约束：![alt text](image-28.png)
   5. 求解器：qpOASES和OSQP![alt text](image-29.png)  
   代码里是这种形式![alt text](qpOASES.png)
   6. 转化成标准形式![alt text](image-31.png)![alt text](image-30.png)
   7. 工具：casdi（看起来很好用）：一种符号框架，可以直接定义符号，调用不同的求解器来求解，也可以用来求解雅可比矩阵，海森矩阵 之类的东西
8. 关节力矩求解：上面的求解结果是足端的力和力矩，但是实际上控制的是关节的力矩，所以还需要求力矩。
   9. 静力学（运动幅度小场景）：依据虚功原理，即足端力的微小位移做的功等于关节微小位移做的功，可以得到足端力到关节力矩（实际控制量）的关系，用雅科比矩阵表示![alt text](image-49.png)
   10. 多刚体动力学（动态场景）
   ...
   11. 对于固定基座模型，可以通过关节力矩来求关节加速度（正动力学），，也可以知道关节加速度来求关节力矩（逆动力学）。但是对于浮动基座模型，知道关节加速度是求不出关节力矩的，还需要body的加速度
   12. reference：推导很复杂，可以用RBDL库来实现。教材推荐：Rigid Body Dynamic Algorithms
13. 全身控制方法
   14. 任务优先级
...
20. 松弛优化：加速度是由WBC求出来的，符合当下的控制，而足端力fc是由模型预测控制求出来的，更符合未来的控制，两者带入到模型会不相等，需要进行松弛优化。求解得到delta_f和delta_q，作为足端力和关节力矩的微小变量来使得原公式的左右两边误差最小
![alt text](image-44.png)
![alt text](image-45.png)
![alt text](image-46.png)
8.  sim2real![alt text](image-47.png)
9.  传统控制缺点![alt text](image-48.png)
